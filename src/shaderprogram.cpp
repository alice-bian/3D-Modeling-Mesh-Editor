...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      ...
      attrJointWeights(-1), attrJointIDs(-1), unifBindMatrices(-1), unifJointTransformationMatrices(-1),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    // additional handles for skeleton shader
    attrJointWeights                = context->glGetAttribLocation(prog, "in_weights");
    attrJointIDs                    = context->glGetAttribLocation(prog, "in_jointIDs");
    unifBindMatrices                = context->glGetUniformLocation(prog, "ua_bindMats");
    unifJointTransformationMatrices = context->glGetUniformLocation(prog, "ua_jointMats");
}
...
void ShaderProgram::setBindMatrix(const std::array<glm::mat4, 100> &model, int num_joints)
{
    useMe();
...
void ShaderProgram::setJointMatrix(const std::array<glm::mat4, 100> &model, int num_joints)
{
    useMe();

    if (unifJointTransformationMatrices != -1) {
        // Pass a 4x4 matrix into a uniform variable in our shader
                        // Handle to the matrix variable on the GPU
        context->glUniformMatrix4fv(unifJointTransformationMatrices,
                        // How many matrices to pass
                           num_joints,
                        // Transpose the matrix? OpenGL uses column-major, so no.
                           GL_FALSE,
